...
public class StatistiqueDao {
    ...
    public List<Object[]> chiffreAffaireAnneeMois(Connexion con,int year)
    {
        List<Object[]> liste = new ArrayList<>();
        try {
            String requete ="select*from chiffreAffaireMoisAnnee where year="+year+"";
            con = new Connexion(requete, "hh");
            ResultSet res = con.getResultset();
            while (con.getResultset().next()) {
                int mois = res.getInt(1);
                int annee = res.getInt(2);
                float montant = res.getFloat(3);
               liste.add(new Object[] {mois,annee,montant});
            }
            return liste;
        } catch (Exception e) {
            return null;
